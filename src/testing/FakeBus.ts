/**
 * In-memory `PulsarBus` substitute for tests — captures every
 ...
 * matching is ported from `crates/pulsar-bus/src/router.rs` — the
...
function buildEventJson(entry: CapturedEmit): string {
	// Mirror the Rust `Event` envelope shape (`crates/pulsar-bus/src/event.rs` —
